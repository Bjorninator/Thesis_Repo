...
public:
    ...
    Generator<bool> handleEvent(TimerEvent const * const evt) {
        ...
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 201, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 1327, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 2216, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 3255, evt->period, evt->deadline);
