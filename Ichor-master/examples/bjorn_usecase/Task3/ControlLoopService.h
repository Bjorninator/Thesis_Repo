...
public:
    ...
    Generator<bool> handleEvent(TimerEvent const * const evt) {
        ...
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 2689, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 6310, evt->period, evt->deadline);
