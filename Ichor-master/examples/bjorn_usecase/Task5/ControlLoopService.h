...
public:
    ...
    Generator<bool> handleEvent(TimerEvent const * const evt) {
        ...
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 696, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 1062, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 1078, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 2161, evt->period, evt->deadline);
