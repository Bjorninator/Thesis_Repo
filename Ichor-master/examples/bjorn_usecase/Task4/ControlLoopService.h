...
public:
    ...
    Generator<bool> handleEvent(TimerEvent const * const evt) {
        ...
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 1003, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 8496, evt->period, evt->deadline);
