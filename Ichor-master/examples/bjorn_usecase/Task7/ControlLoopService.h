...
public:
    ...
    Generator<bool> handleEvent(TimerEvent const * const evt) {
        ...
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 171, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 1779, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 2570, evt->period, evt->deadline);
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(),10 , 4479, evt->period, evt->deadline);
