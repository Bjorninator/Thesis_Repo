...
struct IUsingTimeService {
};

class UsingTimeService final : public IUsingTimeService, public Service<UsingTimeService> {
public:
    UsingTimeService(DependencyRegister &reg, Properties props, DependencyManager *mng) : Service(std::move(props), mng) {
        reg.registerDependency<ILogger>(this, true);
    }
    ~UsingTimeService() final = default;

    bool start() final {
        ICHOR_LOG_INFO(_logger, "UsingTimeService started");
    ...
    Generator<bool> handleEvent(TimerEvent const * const evt) {
        getManager()->pushPrioritisedEvent<Custom1Event>(getServiceId(), 9, 728, evt->period, evt->deadline);
        // period = evt->period;
